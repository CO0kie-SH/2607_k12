...
class K12Service:
    ...
    async def _query_account(
        ...
        for name, path, message in endpoints:
            ...
            endpoint_result = await asyncio.to_thread(self._request_json, url, headers)
    ...
    def _request_json(self, url: str, headers: dict[str, str]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "ok": resp.status_code == 200 and not json_error,
                "content_type": content_type,
    ...
    def _post_empty(self, url: str, access_token: str) -> dict[str, Any]:
        proxies = {"http": self.proxy, "https": self.proxy} if self.proxy else None
        headers = {
            "accept": "*/*",
            "authorization": f"Bearer {access_token}",
        ...
        try:
            resp = requests.post(url, headers=headers, data="", proxies=proxies, timeout=3)
            text = resp.text
            return {
                "status": resp.status_code,
