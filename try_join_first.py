...
def api_get(at: str, path: str) -> dict | None:
    ...
    try:
        resp = requests.get(f"{BASE}{path}", headers=headers, proxies=proxies, timeout=15)
...
def api_post(at: str, path: str) -> bool:
    proxies = {"http": PROXY, "https": PROXY}
    headers = {
        "accept": "*/*",
        "authorization": f"Bearer {at}",
    ...
    try:
        resp = requests.post(f"{BASE}{path}", headers=headers, data="", proxies=proxies, timeout=TIMEOUT)
...
def export_report(at: str, out_dir: Path):
    info = decode_at(at)
    account_id = info.get("account_id", "unknown")
    email = info.get("email", "")

    # API 数据
    me_data = api_get(at, "/backend-api/me") or {}
    accounts_data = api_get(at, "/backend-api/accounts") or {}
...
def main():
    ...
    for i, ws_id in enumerate(ws_ids, 1):
        ...
        # request
        if not api_post(at, f"/backend-api/accounts/{ws_id}/invites/request"):
            print(f"  [{i}/{len(ws_ids)}] {short} X")
            continue
        time.sleep(INTERVAL)

        # accept
        if api_post(at, f"/backend-api/accounts/{ws_id}/invites/accept"):
            print(f"  [{i}/{len(ws_ids)}] {short} OK  -> 停止")
            # 导出报告
            export_report(at, Path(args.out_dir))
            return
        else:
